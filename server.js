...
app.get('/team_stats', function (req, res) {
	...
	// res.render('pages/team_stats', { test: "Hello World" });
});
...
app.get('/player_info', function (req, res) {
	...
	db.task('get-everything', task => {
		...
		.then(data => {
			// console.log(data[0]);
			res.render('pages/player_info', { // template goes here
				my_title: "Player Info", // title goes here
				// context goes here
...
app.post('/player_info/select_player', function (req, res) {
	...
	var query = "select id, name from football_players";
	if (req.body.player_choice != "Select Player") {
		var chosen = "select * from football_players where id=" + req.body.player_choice;
		var num_games = "select count(*) from football_games where " + req.body.player_choice + "=any(football_games.players)";
		db.task('get-everything', task => {
			return task.batch([
				// queries go here
				task.any(query),
			...
			.then(data => {
				...
				// 	if (players[i].id == req.body.player_choice) {
				// 		var player_object = players[i]
				// 	}
				// }
				res.render('pages/player_info', { // template goes here
					my_title: "Player Info", // title goes here
					// context goes here
					players: data[0],
					player_chosen: req.body.player_choice,
					player_object: data[1][0],
					img_src: data[1][0].img_src,
					num_games: data[2][0].count,
				})
			})
			.catch(error => {
				// display error message in case an error
